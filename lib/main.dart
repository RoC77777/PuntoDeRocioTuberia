    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        appBarTheme: AppBarTheme(
          backgroundColor:
              Color(0xFF4064B4), // Color de fondo del AppBar en modo oscuro
      ...
      darkTheme: ThemeData(
        ...
        appBarTheme: AppBarTheme(
          backgroundColor:
              Colors.blueGrey[900], // Color de fondo del AppBar en modo oscuro
          titleTextStyle: TextStyle(
            color: Colors.white, // Color del texto
            fontSize: 24, // Tamaño de la fuente
            fontWeight: FontWeight.bold,
        ...
        elevatedButtonTheme: ElevatedButtonThemeData(
          style: ElevatedButton.styleFrom(
            foregroundColor: Colors.white,
            backgroundColor:
                Colors.blueGrey[900], // Color del texto en el botón
          ),
        ),
      ),

      themeMode: ThemeMode
          .system, // Cambia entre claro y oscuro según la configuración del sistema
      home: MainActivity(),
    );
  }
}
...
class _MainActivityState extends State<MainActivity> {
  ...
  void _calculatePipeTemperature() async {
    ...
    String numTfluido =
        _fluidTempController.text.isEmpty ? "7" : _fluidTempController.text;
    String numQfluido = _flowRateController.text.isEmpty
        ? "0.009318422"
        : _flowRateController.text;
    String numVaire = _airVelocityController.text.isEmpty
        ? "0.1"
        : _airVelocityController.text;

    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();

    double innerRadius =
        double.tryParse(sharedPreferences.getString("innerRadius") ?? "0") ?? 0;
    double outerRadius =
        double.tryParse(sharedPreferences.getString("outerRadius") ?? "0") ?? 0;
    double insulationThickness = double.tryParse(
            sharedPreferences.getString("insulationThickness") ?? "0") ??
        0;
    double length =
        double.tryParse(sharedPreferences.getString("length") ?? "0") ?? 0;
    double thermalConductivityPipe = double.tryParse(
            sharedPreferences.getString("thermalConductivityPipe") ?? "0") ??
        0;
    double thermalConductivityInsulation = double.tryParse(
            sharedPreferences.getString("thermalConductivityInsulation") ??
                "0") ??
        0;

    if (num1String.isNotEmpty &&
        num2String.isNotEmpty &&
        num3String.isNotEmpty &&
        numTfluido.isNotEmpty &&
        numQfluido.isNotEmpty &&
        numVaire.isNotEmpty) {
      // Convertir a doble
      ...
      double u_aire = 0.00001827 *
          ((291.15 + 120) / (temp2K + 120)) *
          pow(temp2K / 291.15, 1.5);
      ...
      double Re_aire =
          Vaire * (2 * (outerRadius + insulationThickness)) / v_aire;
      ...
      double Nu_aire = 0.3 +
          ((0.62 *
                      pow(Re_aire, 0.5) *
                      pow(Pr_aire, 0.33333333333333333333333333333333)) /
                  (pow(
                      1 +
                          pow(0.4 / Pr_aire,
                              0.66666666666666666666666666666667),
                      0.25))) *
              pow(1 + pow(Re_aire / 282000, 0.625), 0.8);
      double h_aire =
          Nu_aire * K_aire / (2 * (insulationThickness + outerRadius));

      double R_conv_agua = 1 / (h_agua * A1);
      double R_cond_tub = log(outerRadius / innerRadius) /
          (2 * pi * length * thermalConductivityPipe);
      double R_cond_aislante =
          log((insulationThickness + outerRadius) / outerRadius) /
              (2 * pi * length * thermalConductivityInsulation);
      double R_conv_aire = 1 / (h_aire * A3);
      double Rt = R_conv_agua + R_cond_tub + R_cond_aislante + R_conv_aire;
      double dQ_dt = (temp2 - Tfluido) / Rt;

      double T_tuberia =
          dQ_dt * (R_conv_agua + R_cond_tub + R_cond_aislante) + Tfluido;
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                    labelText: "Temperatura del fluido(°C), default: 7"),
                keyboardType: TextInputType.number,
              ),
              TextField(
                controller: _flowRateController,
                decoration: InputDecoration(
                    labelText: "Caudal(m³/s), default: 0.009318422"),
                keyboardType: TextInputType.number,
              ),
              TextField(
                controller: _airVelocityController,
                decoration: InputDecoration(
                    labelText: "Velocidad del aire(m/s), default: 0.1"),
                keyboardType: TextInputType.number,
              ),
              ...
              if (_showImage) Image.asset("assets/tuberia_cond.png"),
              if (_showImage2) Image.asset("assets/tuberia_sec.png"),
